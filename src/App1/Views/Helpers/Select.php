...
class Select
{
    ...
    {
        $select = self::SELECT_OPEN_VALUE . $this->_id . '" name = "' . $this->_name . '">';
        $options = $this->getOptions($this->_params);
    ...
    {
        $optionsContent = self::OPTION_OPEN_VALUE . self::OPTION_END
                . self::OPTION_DEFAULT_MESSAGE . self::OPTION_CLOSE_VALUE
                . self::CR;
        $tupple = Arrayproto::getTupple($options);
        foreach ($tupple as $key => $value) {
            $selected = ($value == $this->_selected) ? self::OPTION_SELECTED : self::OPTION_END;
    ...
    {
        return self::OPTION_OPEN_VALUE . $value . $selected . $key . self::OPTION_CLOSE_VALUE;
    }
}
